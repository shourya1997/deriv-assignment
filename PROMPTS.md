...
## `code/` — Runnable prototype (optional deliverable)

**Prompt (paraphrased), the architecture pivot:** "the overall app should be dockerised and i
should be able to run this on my laptop", "Use TDD style of development", "Airflow is also
required", "These should be config driven: OLTP -> OLAP, layer 1 -> layer 2 -> layer 3", "per
table test", "dags should be auto generated."

**AI's initial plan (pre-pivot):** one hand-written loader module and one hand-authored DAG
file per named source table (`vendor_deposits_pipeline.py`, `client_trades_pipeline.py`, etc.).

**Decided:** the user overrode this with the config-driven/auto-generated requirement. Final
architecture: one declarative YAML schema per table (`code/config/tables/*.yml`), a generic
3-layer engine (`deriv_pipeline/layers/`) dispatching on config, a single `dag_factory.py`
generating one DAG per config entry (two hand-authored DAGs kept, for stated reasons — see
`code/ARCHITECTURE_DECISIONS.md`), and per-table tests generated by parametrizing over the
same config rather than hand-written per table name.

**Prompt (paraphrased), process:** "in the plan add, after each phase run dual review Opus and
Sonnet", "Use TASK, PROGRESS, Architecture Design Decision markdown files", "git commit after
each phase", "For running simple tasks... use Haiku Model, Use Sonnet to guide and correct."

**Decided:** dual Opus+Sonnet review gate after every phase; `code/TASK.md`/`PROGRESS.md`/
`ARCHITECTURE_DECISIONS.md` as the restart-safe source of truth (separate from this file);
Haiku for deterministic verification steps (running tests, checking exit codes), Sonnet for
implementation/judgment/diagnosis. Two full rounds of independent dual review were run against
the plan itself (against real `sql/`/`data/` files) before implementation started — see G1/G2
below, both found only because of that review.

**G1/G2 gaps found by dual review (real bugs, not design changes):**
- **G1** — `sql/03_cdc_apply.sql` writes `quarantine.rejected_rows` and
  `sql/04_backfill_reset.sql` reads `raw.client_profile_changes`, but no `raw`/`staging`/
  `quarantine`/`data_quality` schema or those tables exist anywhere in `sql/` — `CREATE SCHEMA
  warehouse` itself is also missing. Fixed with gap-fill migration files interleaved with
  `sql/01`-`04` in dependency order.
- **G2** — `dim_client_risk_snapshot`-derived `risk_snapshot_key` is `NOT NULL` on both fact
  tables, but re-deriving actual date ranges from `data/` (not trusting an earlier hand-written
  summary) showed **all 62 fact rows**, not just a March-only subset, predate the earliest CDC
  event (`lsn 1001`, `2024-10-01`) — `resolve_risk_snapshot_key()` returns `NULL` for all of
  them and the constraint fails on first load. Fixed with a baseline snapshot seed at
  `source_lsn=0`, idempotent via `ON CONFLICT ON CONSTRAINT uq_client_lsn DO NOTHING`,
  excluding CL030 (whose only CDC event is its own `insert`, so a "baseline" would fabricate
  history), sequenced strictly before any `apply_cdc_event` call.

**Numeric-claims policy reversal:** an earlier plan draft hard-coded specific row counts for
edge-case tests; a verification pass on those numbers did not complete cleanly, so the policy
changed to "every count assertion is recomputed from `data/` at test time," never a bare
literal.

**`docker-entrypoint-initdb.d` rejected** in favor of an explicit migration manifest tracked in
`schema_migrations` — not re-runnable, can't interleave with gap-fill DDL, unusable from
pytest.

**GE scoped down** to one real suite (`staging_vendor_deposits`) run via a plain
`PythonOperator`, plus config-declared SQL assertions for every other table's DQ — the
`airflow-provider-great-expectations` operator and GE/Airflow version coupling were identified
as the single most likely cause of a broken `docker compose up`; a deliberate, disclosed scope
reduction, not a silent drop of the requirement.
